...
def get_best_model():
    ...
    for preferred in ["gpt-4o", "gpt-4-turbo", "gpt-4", "gpt-3.5-turbo"]:
        if any(preferred in m["id"] for m in models["data"]):
            return preferred
    raise Exception("No available GPT model found.")
...
def generate_grocery_image(prompt: str) -> str:
    preferred_models = ["dall-e-3", "gpt-image-1"]
    available_models = [
        m["id"] for m in openai.Model.list()["data"]
        if any(pm in m["id"] for pm in preferred_models)
    ]
    model_to_use = available_models[0] if available_models else "gpt-image-1"
